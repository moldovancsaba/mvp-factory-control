...
# WarRoom Docker portability preflight.
...
check_required_env_key() {
  ...
  if [ -z "$line" ]; then
    fail "Missing required key in apps/warroom/.env: ${key}. Remediation: copy apps/warroom/.env.example and set ${key}."
    return
  fi
  value="${line#*=}"
  compact="$(printf "%s" "$value" | tr -d '[:space:]')"
  if [ -z "$compact" ]; then
    fail "Empty required key in apps/warroom/.env: ${key}. Remediation: set a non-empty value."
    return
  fi
  case "$value" in
...
echo "WarRoom Docker portability preflight"
echo "Repo: $REPO_ROOT"
echo
...
if [ -f "$ENV_FILE" ]; then
  pass "WarRoom env file exists: apps/warroom/.env"
  ...
  check_required_env_key "$ENV_FILE" "WARROOM_GITHUB_TOKEN"
else
  fail "WarRoom env file is missing: apps/warroom/.env. Remediation: copy apps/warroom/.env.example to apps/warroom/.env and fill required keys."
fi

if command -v lsof >/dev/null 2>&1; then
  check_port_listener "$DB_PORT" "WARROOM_DB_PORT"
  check_port_listener "$APP_PORT" "WARROOM_APP_PORT"
else
