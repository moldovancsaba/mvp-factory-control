...
# One-command Docker bootstrap for the internal control app:
...
HEALTH_TIMEOUT_SECONDS="${MVP_FACTORY_CONTROL_DOCKER_HEALTH_TIMEOUT_SECONDS:-120}"
...
echo "Internal control app Docker bootstrap"
echo "Repo: $REPO_ROOT"
...
echo "[STEP] Stopping existing internal control app stack (if running)"
dc down --remove-orphans >/dev/null 2>&1 || true
echo "[PASS] Existing stack stopped (or was not running)"
echo

echo "[STEP] Reclaiming configured host ports"
reclaim_port "$DB_PORT" "MVP_FACTORY_CONTROL_DB_PORT"
reclaim_port "$APP_PORT" "MVP_FACTORY_CONTROL_APP_PORT"
echo

echo "[STEP] Starting database service"
dc up -d mvp-factory-control-db
wait_for_health "mvp-factory-control-db" "$HEALTH_TIMEOUT_SECONDS"
echo

echo "[STEP] Applying Prisma migrations (idempotent)"
dc run --rm --no-deps mvp-factory-control-app npx prisma migrate deploy
echo "[PASS] Prisma migrations applied"
echo

echo "[STEP] Starting app service"
dc up -d mvp-factory-control-app
wait_for_health "mvp-factory-control-app" "$HEALTH_TIMEOUT_SECONDS"
echo
...
if curl -fsS "http://127.0.0.1:${APP_PORT}/signin" >/dev/null; then
  ...
  docker logs --tail 80 mvp-factory-control-app || true
  exit 1
fi

echo
echo "[PASS] Internal control app Docker bootstrap completed."
