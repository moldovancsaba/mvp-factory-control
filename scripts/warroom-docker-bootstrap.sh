...
# One-command Docker bootstrap for WarRoom:
...
dc up -d warroom-db
wait_for_health "warroom-db" "$HEALTH_TIMEOUT_SECONDS"
echo

echo "[STEP] Applying Prisma migrations (idempotent)"
dc run --rm --no-deps warroom-app npx prisma migrate deploy
echo "[PASS] Prisma migrations applied"
echo

echo "[STEP] Starting app service"
dc up -d warroom-app
wait_for_health "warroom-app" "$HEALTH_TIMEOUT_SECONDS"
echo
...
if curl -fsS "http://127.0.0.1:${APP_PORT}/signin" >/dev/null; then
  ...
  docker logs --tail 80 warroom-app || true
  exit 1
fi

echo
echo "[PASS] WarRoom Docker bootstrap completed."
