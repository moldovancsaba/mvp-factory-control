...
# Internal control app Docker portability preflight.
...
ENV_FILE="$REPO_ROOT/apps/mvp-factory-control/.env"
DB_PORT="${MVP_FACTORY_CONTROL_DB_PORT:-3579}"
APP_PORT="${MVP_FACTORY_CONTROL_APP_PORT:-3577}"
...
check_required_env_key() {
  ...
  if [ -z "$line" ]; then
    fail "Missing required key in apps/mvp-factory-control/.env: ${key}. Remediation: copy apps/mvp-factory-control/.env.example and set ${key}."
    return
  fi
  value="${line#*=}"
  compact="$(printf "%s" "$value" | tr -d '[:space:]')"
  if [ -z "$compact" ]; then
    fail "Empty required key in apps/mvp-factory-control/.env: ${key}. Remediation: set a non-empty value."
    return
  fi
  case "$value" in
...
echo "Internal control app Docker portability preflight"
echo "Repo: $REPO_ROOT"
echo
...
if [ -f "$ENV_FILE" ]; then
  pass "Internal control app env file exists: apps/mvp-factory-control/.env"
  ...
  check_required_env_key "$ENV_FILE" "MVP_FACTORY_CONTROL_GITHUB_TOKEN"
else
  fail "Internal control app env file is missing: apps/mvp-factory-control/.env. Remediation: copy apps/mvp-factory-control/.env.example to apps/mvp-factory-control/.env and fill required keys."
fi

if command -v lsof >/dev/null 2>&1; then
  check_port_listener "$DB_PORT" "MVP_FACTORY_CONTROL_DB_PORT"
  check_port_listener "$APP_PORT" "MVP_FACTORY_CONTROL_APP_PORT"
else
