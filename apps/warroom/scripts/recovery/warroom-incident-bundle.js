...
function parseArgs(argv) {
  const args = {
    issue: null,
    sinceHours: 24 * 7,
    out: path.resolve(process.cwd(), ".warroom/recovery/incident-bundle.json"),
    mode: process.env.WARROOM_RECOVERY_DRILL_MODE || "AUTO"
  };
...
main().catch((error) => {
  console.error("[warroom-incident-bundle] failed:", error.message || error);
  process.exitCode = 1;
});
