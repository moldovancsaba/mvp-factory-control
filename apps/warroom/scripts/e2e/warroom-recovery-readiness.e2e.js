...
async function stageBackup({ backupDir, mode, sinceHours }) {
  ...
  const snapshot = {
    ...
    envPresence: {
      ...
      WARROOM_GITHUB_TOKEN: Boolean(process.env.WARROOM_GITHUB_TOKEN),
...
async function main() {
  ...
  const runId = `warroom-recovery-readiness-e2e-${new Date().toISOString()}`;
  const issueNumber = 120;
  const sinceHours = 24 * 7;
  const mode = String(process.env.WARROOM_RECOVERY_DRILL_MODE || "AUTO").toUpperCase();
  ...
  const root = await fsp.mkdtemp(path.join(os.tmpdir(), "warroom-recovery-drill-"));
...
main().catch((error) => {
  console.error("[warroom-recovery-readiness-e2e] failed:", error.message || error);
  process.exitCode = 1;
});
