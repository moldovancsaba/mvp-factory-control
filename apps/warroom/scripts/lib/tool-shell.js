...
function buildResourceLimits(env) {
  return {
    defaultTimeoutMs: clampInt(
      env.WARROOM_SHELL_DEFAULT_TIMEOUT_MS,
      DEFAULT_TIMEOUT_MS,
      500,
      MAX_TIMEOUT_MS
    ),
    maxTimeoutMs: clampInt(env.WARROOM_SHELL_MAX_TIMEOUT_MS, MAX_TIMEOUT_MS, 1_000, 600_000),
    maxOutputBytes: clampInt(
      env.WARROOM_SHELL_MAX_OUTPUT_BYTES,
      DEFAULT_MAX_OUTPUT_BYTES,
      4_096,
      MAX_OUTPUT_BYTES
    ),
    maxCommandChars: clampInt(
      env.WARROOM_SHELL_MAX_COMMAND_CHARS,
      DEFAULT_MAX_COMMAND_CHARS,
      64,
      MAX_COMMAND_CHARS
    ),
    cancelPollMs: clampInt(env.WARROOM_SHELL_CANCEL_POLL_MS, DEFAULT_CANCEL_POLL_MS, 100, 5_000),
    killGraceMs: clampInt(env.WARROOM_SHELL_KILL_GRACE_MS, DEFAULT_KILL_GRACE_MS, 200, 10_000),
    maxCpuSeconds: clampInt(env.WARROOM_SHELL_MAX_CPU_SECONDS, DEFAULT_MAX_CPU_SECONDS, 1, 3600),
    maxMemoryKb: clampInt(
      env.WARROOM_SHELL_MAX_MEMORY_KB,
      DEFAULT_MAX_MEMORY_KB,
      65_536,
      8_388_608
    ),
    maxProcessCount: clampInt(
      env.WARROOM_SHELL_MAX_PROCESS_COUNT,
      DEFAULT_MAX_PROCESS_COUNT,
      0,
      4096
    ),
    shellBinary: asTrimmed(env.WARROOM_SHELL_BINARY) || DEFAULT_SHELL_BINARY
  };
}

function buildAllowedEnvKeys(env) {
  const extra = parseKeyList(env.WARROOM_SHELL_ENV_ALLOWLIST);
...
async function resolveShellToolContext(options = {}) {
  ...
  const sessionRootBase =
    ...
    asTrimmed(env.WARROOM_SHELL_SESSION_ROOT) ||
    path.join(process.cwd(), ".warroom", "shell-sessions");
