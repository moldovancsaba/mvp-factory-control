...
function readSecret(explicit) {
  const secret = String(
    explicit || process.env.WARROOM_TOOL_APPROVAL_SECRET || process.env.NEXTAUTH_SECRET || ""
  ).trim();
  if (!secret) {
    throw new Error(
      "Tool approval secret is not configured. Set WARROOM_TOOL_APPROVAL_SECRET (or NEXTAUTH_SECRET)."
    );
  }
  return secret;
}
