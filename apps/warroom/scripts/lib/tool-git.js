...
async function createPullRequest(context, args, repoRoot, headBranch) {
  if (!context.githubToken) {
    throw new ToolGitError("TOKEN_MISSING", "git.pr.create denied: WARROOM_GITHUB_TOKEN is missing.");
  }
...
async function resolveGitToolContext(options = {}) {
  ...
  return {
    ...
    timeoutMs: clampInt(env.WARROOM_GIT_TIMEOUT_MS, DEFAULT_TIMEOUT_MS, 1_000, 120_000),
    maxOutputBytes: clampInt(
      env.WARROOM_GIT_MAX_OUTPUT_BYTES,
      DEFAULT_MAX_OUTPUT_BYTES,
      4_096,
      1_048_576
    ),
    protectedBranches: new Set(parseProtectedBranches(env.WARROOM_GIT_PROTECTED_BRANCHES)),
    env,
    repoOwner: asTrimmed(env.WARROOM_GITHUB_REPO_OWNER) || "moldovancsaba",
    repoName: asTrimmed(env.WARROOM_GITHUB_REPO_NAME) || "mvp-factory-control",
    githubToken:
      asTrimmed(env.WARROOM_GITHUB_TOKEN) ||
