...
export async function bootstrapWarRoomProjectAction() {
  const auth = await requireRbacAccess({
    action: "PRODUCTS_BOOTSTRAP_WARROOM_PROJECT",
    allowedRoles: ["ADMIN", "OPERATOR"],
    entityType: "PROJECT_SETTINGS",
    entityId: "WarRoom"
  });
  ...
  const repoOwner = process.env.WARROOM_TASK_REPO_OWNER || "moldovancsaba";
  const repoName = process.env.WARROOM_TASK_REPO_NAME || "mvp-factory-control";
  ...
  if (process.env.WARROOM_ENABLE_PRODUCT_OPTION_SYNC === "1") {
  ...
  await upsertProjectSetting({
    ...
    vars: [
      { key: "APP_PATH", value: path.join(repoRoot, "apps", "warroom") },
      { key: "ROADMAP_DOC", value: path.join(repoRoot, "docs", "WARROOM_STATUS_BRAINDUMP.md") },
      { key: "HANDOVER_DOC", value: path.join(repoRoot, "docs", "WARROOM_HANDOVER.md") },
      { key: "SETUP_DOC", value: path.join(repoRoot, "docs", "WARROOM_SETUP.md") }
    ]
  }, {
    auditContext: {
      actorRole: `RBAC_${auth.role}`,
  ...
  revalidatePath("/products");
  revalidatePath("/products/WarRoom");
  revalidatePath("/settings");
}
