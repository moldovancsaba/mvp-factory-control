...
function staleRunningThresholdMs() {
  const leaseTtl = Number(process.env.WARROOM_ORCHESTRATOR_LEASE_TTL_MS || "20000");
  const fallback = Math.max((Number.isFinite(leaseTtl) ? leaseTtl : 20_000) * 2, 30_000);
  const raw = Number(process.env.WARROOM_ORCHESTRATOR_STALE_RUNNING_MS || String(fallback));
