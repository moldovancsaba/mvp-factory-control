...
import { readWarRoomSettings } from "@/lib/settings-store";
...
const SAFE_RUNTIME_KEYS = new Set([
  "WARROOM_RUNTIME_ENDPOINT",
  "WARROOM_RUNTIME_MODEL",
  "WARROOM_RUNTIME_API_KEY_ENV",
  "WARROOM_RUNTIME_TIMEOUT_MS",
  "WARROOM_RUNTIME_LOCAL_ENDPOINT",
  "WARROOM_RUNTIME_LOCAL_MODEL",
  "WARROOM_RUNTIME_CLOUD_ENDPOINT",
  "WARROOM_RUNTIME_CLOUD_MODEL",
  "WARROOM_RUNTIME_CLOUD_API_KEY_ENV"
]);
...
function applyRuntimeOverrides(params: {
  ...
  const runtimeEndpointKey =
    params.runtime === "LOCAL" ? "WARROOM_RUNTIME_LOCAL_ENDPOINT" : "WARROOM_RUNTIME_CLOUD_ENDPOINT";
  const runtimeModelKey =
    params.runtime === "LOCAL" ? "WARROOM_RUNTIME_LOCAL_MODEL" : "WARROOM_RUNTIME_CLOUD_MODEL";
  const runtimeApiEnvKey =
    params.runtime === "CLOUD" ? "WARROOM_RUNTIME_CLOUD_API_KEY_ENV" : "";

  applyStringValue([runtimeEndpointKey, "WARROOM_RUNTIME_ENDPOINT"], (value) => {
    params.effective.endpoint = value;
  });

  applyStringValue([runtimeModelKey, "WARROOM_RUNTIME_MODEL"], (value) => {
    params.effective.model = value;
  });

  if (params.runtime === "CLOUD") {
    applyStringValue([runtimeApiEnvKey, "WARROOM_RUNTIME_API_KEY_ENV"], (value) => {
      params.effective.apiKeyEnv = value;
    });
  }

  const timeoutRaw = getValue("WARROOM_RUNTIME_TIMEOUT_MS");
  if (timeoutRaw) {
    if (canApply("WARROOM_RUNTIME_TIMEOUT_MS")) {
      params.effective.requestTimeoutMs = clampTimeout(timeoutRaw, params.effective.requestTimeoutMs);
      appliedKeys.push("WARROOM_RUNTIME_TIMEOUT_MS");
    } else {
      ignoredKeys.push("WARROOM_RUNTIME_TIMEOUT_MS");
    }
  }
...
export async function resolveRuntimeConfigForTask(params: {
  ...
  const effective: RuntimeConfigEffective = {
    ...
    requestTimeoutMs: clampTimeout(process.env.WARROOM_WORKER_REQUEST_TIMEOUT_MS, 60_000)
  };
  ...
  sourceChain.push({
    ...
    appliedKeys:
      runtime === "LOCAL"
        ? ["OLLAMA_BASE_URL", "OLLAMA_MODEL", "WARROOM_WORKER_REQUEST_TIMEOUT_MS"]
        : [
            "OPENAI_BASE_URL",
            "OPENAI_MODEL",
            "OPENAI_API_KEY",
