...
function resolveSenderAuth(senderEmail: string): SenderAuthResult {
  const blocked = parseAddressList(process.env.WARROOM_EMAIL_BLOCKED_SENDERS);
  const trusted = parseAddressList(process.env.WARROOM_EMAIL_TRUSTED_SENDERS);
  const requireTrusted = isTruthy(process.env.WARROOM_EMAIL_REQUIRE_TRUSTED, true);
  ...
  if (blocked.has(email)) {
    return { allowed: false, reason: "Sender is blocked by WARROOM_EMAIL_BLOCKED_SENDERS." };
  }
  if (requireTrusted && !trusted.has(email)) {
    return {
      allowed: false,
      reason: "Sender is not in WARROOM_EMAIL_TRUSTED_SENDERS."
    };
  }
...
function computeRetryDelayMs(attempt: number) {
  const base = clampInt(process.env.WARROOM_EMAIL_RETRY_BASE_MS, 1_000, 100, 60_000);
  const max = clampInt(process.env.WARROOM_EMAIL_RETRY_MAX_MS, 15_000, base, 300_000);
...
export async function handleInboundEmail(payload: InboundEmailPayload) {
  ...
  const maxAttempts = clampInt(process.env.WARROOM_EMAIL_RETRY_MAX_ATTEMPTS, 3, 1, 10);
