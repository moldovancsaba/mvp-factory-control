...
function getGithubToken() {
  ...
  if (!token) {
    throw new Error(
      "Missing GitHub token. Set WARROOM_GITHUB_TOKEN (recommended) or GITHUB_TOKEN."
    );
  }
  return token;
}
...
export async function getProjectMeta() {
  const owner = process.env.WARROOM_GITHUB_PROJECT_OWNER || "moldovancsaba";
  const number = Number(process.env.WARROOM_GITHUB_PROJECT_NUMBER || "1");
